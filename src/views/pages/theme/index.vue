      ...
      <div class="container">
        <div class="circle-container">
          ...
          <div class="sun-container">
            <div class="sun" @click="onChangeMorningOrEvening"></div>
          </div>
...
const onBack = (): void => {
  ...
  if (lastRouter) {
    ...
    router.push('/login'); // 请替换 'home' 为你的首页路由名称或路径
  }
};

const onChangeMorningOrEvening = (e: MouseEvent) => {
      e.stopPropagation();
      changeToggleClass('.page', 'container-night')
      changeToggleClass('.sky-night', 'sky-night-fade')
      changeToggleClass('p', 'text-color')
      changeToggleClass('h1', 'text-color')
      changeToggleClass('.ocean-night', 'ocean-night-fade');
      changeToggleClass('.moon', 'moon-fade');
      changeToggleClass('.ocean', 'animation-stop');
      changeToggleClass('.bird', 'birds-fly');
      changeToggleClass('.boat', 'boat-sail');
      changeToggleClass('.mountain-top', 'mountain-top-night');
      changeToggleClass('.mountain-top > *', 'mountain-top-night');
      changeToggleClass('.mountain-middle', 'mountain-middle-night');
      changeToggleClass('.mountain-middle > *', 'mountain-middle-night');
      changeToggleClass('.mountain-back', 'mountain-back-night');
      changeToggleClass('.mountain-back > *', 'mountain-back-night');
      changeToggleClass('.stars', 'stars-fade');
...
const changeToggleClass = (targetClass: string, className: string) => {
  // 遍历所有匹配的元素并切换类名
  document.querySelectorAll(targetClass).forEach((page) => {
    page.classList.toggle(className);
  });
};

const delayClass = (targetClass: string, delay: number) => {
  const clouds =  document.querySelectorAll(targetClass)
  clouds.forEach((el) => {
    var _el = el as HTMLElement;
    _el.style.opacity = '0';
  });
  ...
  setTimeout(() => {
    clouds.forEach((el) => {
      var _el = el as HTMLElement;
      // 检查元素是否已经可见，然后切换可见性
      _el.style.transition = "opacity 2.5s";
      _el.style.opacity = '1';
    });
  }, delay);
};
